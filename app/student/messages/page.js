    ...
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (studentId) {
          ...
          if (sDoc.exists()) myClassId = sDoc.data().classId;
        }

        // 3. Trouver les enseignants de sa classe (subjectAssignments)
        const teacherIds = new Set();
        if (myClassId) {
          const assignSnap = await getDocs(collection(db, "schools", SCHOOL_ID, "classes", myClassId, "subjectAssignments"));
          assignSnap.docs.forEach((a) => { if (a.data().teacherId) teacherIds.add(a.data().teacherId); });
        }

        // 4. Charger les users (noms)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="rounded-2xl p-6 text-white shadow-lg"
        ...
        <p className="text-green-100 text-sm">Échangez avec vos enseignants.</p>
      </div>
        ...
        <div className="flex-1 flex flex-col">
          {showNew ? (
            <div className="p-4 overflow-y-auto">
              <h3 className="font-semibold text-gray-700 mb-3 text-sm">Choisir un enseignant</h3>
              {contacts.length === 0 ? (
                <p className="text-gray-400 text-sm">Aucun enseignant disponible.</p>
              ) : (
