...
export default function AdminSidebar({ open, onClose }) {
  ...
  const links = [
    ...
    { href: "/admin/timetable", label: "Emploi du temps", icon: "📅" },
      ...
      >
        <div className="px-6 py-5 border-b border-white/15 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-[#0AAAFF] to-[#5FBF56] rounded-xl flex items-center justify-center font-bold">
              E
            </div>
            <div>
              <p className="font-bold text-sm">EDU-SENSE</p>
              <p className="text-xs text-[#7FD1FF]">IA Platform</p>
            </div>
          </div>
          {/* Bouton fermer (visible sur mobile seulement) */}
